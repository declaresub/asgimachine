...
|---|---|---|---|---|
| **HTTP semantics** (caching, conditional requests, negotiation, `405`/`406`/`412`/`428`) | **Correct by default** | Hand-wired per route | Hand-wired per route | Hand-wired per view |
...
below it, the incumbents do as well or better.

## vs FastAPI — the closest neighbor
...
For a large fraction of APIs, **FastAPI is the right call.** asgimachine wins on a
specific axis; make sure that axis is your problem.
